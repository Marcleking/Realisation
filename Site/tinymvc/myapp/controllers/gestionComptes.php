...
class GestionComptesController extends TinyMVC_Controller
{
            ...
            if ($_SESSION['user']->getType() == "Gestionnaire") {
                ...
                foreach ($listEmploye as $x_value) {
                    $listHtml .= '<hr><dd><a href="#panel' .
                                strtr($x_value["courriel"], array("." => "", "@" => "")) .'">';
                    $listHtml .=  $x_value["prenom"] ." ". $x_value["nom"];
                    ...
                    $listHtml .= '</a>';
                    $listHtml .= '<div id="panel'.
                                strtr($x_value["courriel"], array("." => "", "@" => "")) .'" class="content">';
                    $listHtml .= '<div class="left"> Nom: '. $x_value["prenom"] ." ". $x_value["nom"]. '</div>';
                    $listHtml .= '<div class="right"> Adresse: '. $x_value["numeroCivique"] .", ". $x_value["rue"];
                    $listHtml .= '<br />'.$x_value["ville"]. ' '. $x_value["codePostal"] .'</div>';
                    $listHtml .= '<br />Courriel: '.$x_value["courriel"];
                    $listHtml .= '<br />Type Employé: '. $x_value["typeEmploye"];
                    ...
                    $listHtml .= '<br /><div class="right"><a href="'.url.'/modificationsAdmin?courriel='. $x_value["courriel"] .'"  class="button tiny">Modifier</a> <a href="'.url.'/gestionComptes?suppId='. $x_value["courriel"] .'" class="button alert tiny">Supprimer</a></div><br /></div></dd>';
                }
